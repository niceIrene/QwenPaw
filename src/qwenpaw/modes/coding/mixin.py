...
# The minimal template without the recall tool: CodeAct repl-only under the
# native context strategy (``apply_codeact_mode(scroll=False)``), where
# ``recall_history_python`` and ``ms`` do not exist.
...
You call one tool directly: `repl_exec`, a persistent kernel where all work
happens. Cells run strictly in order in one namespace, and variables persist
across cells; if a cell returns `kernel_busy` because a long cell is still
running, wait and retry rather than resubmitting. Every other capability is
a governed tool reached from *inside* a `repl_exec` cell as
`paw.tools.<name>(...)`, never as a top-level tool call; whenever an
instruction refers to such a tool by bare name, invoke it as
`paw.tools.<name>(...)`. Use `dir(paw.tools)` or `paw.list_tools()` to see
what is available.
