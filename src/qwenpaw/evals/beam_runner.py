...
_STRUCTURED_PROMPT = (
    ...
    f"and kind='{BEAM_HISTORY_KIND}'. Start with k=5 or k=10 and increase it "
    "only when the evidence is insufficient. If a question combines multiple "
    "named systems, efforts, or entities, search each one separately instead "
    "of putting every name into one AND query. Extract one directly relevant "
    "value for each part, preserve its unit, and only then calculate. A "
    "search result already includes the "
    ...
    "evidence that the user adopted it. Preserve exact numbers, units, and "
    "version labels from the most directly relevant user evidence; do not "
    "replace them with illustrative values. Do not add repeated historical "
    "mentions as separate quantities unless the question explicitly asks for "
    "that. When a fact changed, use the latest user evidence only after "
    "confirming that the rows describe the same project and the same fact, "
    "not merely similarly named work. "
...
_PYTHON_PROMPT = (
    ...
    f"ms.search(QUERY, all_agents=True, kind='{BEAM_HISTORY_KIND}', k=10). "
    "Start with k=5 or k=10 and increase it only when the evidence is "
    "insufficient. If a question combines multiple named systems, efforts, "
    "or entities, search each one separately instead of putting every name "
    "into one AND query. Extract one directly relevant value for each part, "
    "preserve its unit, and only then calculate. "
    ...
    "adopted it. Preserve exact numbers, units, and version labels from the "
    "most directly relevant user evidence; do not replace them with "
    "illustrative values. Do not add repeated historical mentions as "
    "separate quantities unless the question explicitly asks for that. When "
    "a fact changed, use the latest user evidence only after confirming that "
    "the rows describe the same project and the same fact, not merely "
    "similarly named work. Base your answer only on "
